...
//        list.add(courseCondition4);
    ...
    private static List<CourseCondition> getSortCourses(List<CourseCondition> list){

        Collections.sort(list,CompareUtil.createComparator(-1,"weight","section","join"));

        return list;
    }

    //对课程数据进行排课
    public static String[] arrange(List<CourseCondition> list,Map<Integer,String > map){

        initTimeTable(timeTable,map);

        //获取优先级降序的  课程
        List<CourseCondition> sortCourses=getSortCourses(list);

        for(CourseCondition courseCondition:sortCourses)
            place(courseCondition);

        return timeTable;
    }

    //该课程是否可以被放置
    private static boolean place(CourseCondition course){
